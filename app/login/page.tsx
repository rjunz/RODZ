...
export default async function LoginPage({
  ...
  const hasSession = !!jar.get('rodz_session');
  if (hasSession) {
    redirect('/dashboard');
  }
  ...
  // Se você aplicou o suporte a `?next=` na API, troque o href abaixo para:
  // const loginHref = '/api/auth/discord/login?next=/dashboard';
  ...
  return (
    <main className="wrap">
      <div className="card">
        ...
        {!!errorMsg && (
          <div className="error">
            {errorMsg === '1' ? 'Não foi possível autenticar.' : errorMsg}
          </div>
        )}
